...
class DQNAgent:
    ...
    def __init__(self):
        ...
        # -------------------------------------------------
        # 🔥 SAFE PROFILE HANDLING (CRITICAL FIX)
        # -------------------------------------------------
        profile = get_system_profile() or {}

        memory = profile.get("memory") or {}
        ram = memory.get("total_gb", 8)

        # -------------------------------------------------
        # MODEL SIZE BASED ON SYSTEM
        # -------------------------------------------------
        ...
        # -------------------------------------------------
        # 🔥 SAFE OPTIMIZER CONFIG
        # -------------------------------------------------
        opt = get_optimizer() or {}
    ...
    def encode_state(self, m):
        ...
        return np.array([
            ...
            m.get("memory", 0) / 100,
            m.get("disk", 0) / 100,
            (m.get("network") or {}).get("latency", 0) / 100,
            (m.get("network") or {}).get("throughput", 0) / 100,
